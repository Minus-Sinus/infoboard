# turn on debug mode: run
# export FLASK_ENV=development

# ****** Next steps ******
# typeahead not showing up
# search.html is working correctly
# check mashup for occurences of "q" to see if something is missing
...
def dashboard():
    ...
    if request.method == "POST":
        ...
        SelectionName = str(request.form.get("name_symbol"))
        ...
        db.execute("UPDATE users SET Last_stock=:new_sym WHERE id=:uid", uid=session["user_id"], new_sym=new_symbol[0])
        symbol = new_symbol[0]
    ...
    for c in range(1,4): # later: 1,4
        for r in range(1,4): # later 1,4
...
@app.route("/search") # Used for typeahead
def search():
    ...
    try:
        ...
        raise ValueError('Missing parameter name_symbol in search')
        return jsonify([])

    # search in postal codes of database:
    rows = db.execute("SELECT stockname || ' (' || FROM stocks \
                      WHERE symbol LIKE :name_symbol \
                      OR stock_name LIKE :name_symbol", name_symbol=str_search)

    print ("Debug: typeahead search: first result:")
    print (rows[:1])
